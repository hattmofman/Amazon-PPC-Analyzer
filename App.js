// Browser-compatible version - using global variables from CDN
const { useState, useMemo, useEffect } = React;
const { Upload, TrendingUp, TrendingDown, DollarSign, Target, MousePointer, AlertTriangle, CheckCircle, XCircle, Filter, Search, Download, ChevronDown, ChevronRight, ArrowUpDown, Settings, Loader2, FileSpreadsheet } = window.LucideIcons;
...
function App({ session, supabase }) {
  ...
  const analyzeData = (rawData) => {
    const validData = rawData.filter(row => {
      const hasSpend = parseNumericValue(getValue(row, ['Spend', 'Cost'])) > 0;
      const hasImpressions = parseNumericValue(getValue(row, ['Impressions'])) > 0;
      const hasClicks = parseNumericValue(getValue(row, ['Clicks'])) > 0;
    ...
    const totalSpend = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Spend', 'Cost'])), 0);
    const totalSales = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Sales', 'Revenue'])), 0);
    const totalClicks = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Clicks'])), 0);
    const totalImpressions = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Impressions'])), 0);
    const totalOrders = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Orders'])), 0);
    
    // Campaign analysis
    const campaignMap = new Map();
    validData.forEach(row => {
      const campaignName = String(getValue(row, ['Campaign Name (Informational only)', 'Campaign Name', 'Campaign'])).trim() || 'Unknown';
      ...
      campaign.spend += parseNumericValue(getValue(row, ['Spend', 'Cost']));
      campaign.sales += parseNumericValue(getValue(row, ['Sales', 'Revenue']));
      campaign.clicks += parseNumericValue(getValue(row, ['Clicks']));
      campaign.impressions += parseNumericValue(getValue(row, ['Impressions']));
      campaign.orders += parseNumericValue(getValue(row, ['Orders']));
    ...
    const autoVsManual = analyzeAutoVsManual(validData);
    const matchTypeAnalysis = analyzeMatchTypes(validData);
    const keywordAnalysis = analyzeKeywords(validData);
    const wastedSpend = analyzeWastedSpend(validData);
  ...
  const analyzeAutoVsManual = (data) => {
    ...
    data.forEach(row => {
      const targetingType = String(getValue(row, ['Targeting Type', 'Match Type'])).toLowerCase();
      ...
      groups[type].spend += parseNumericValue(getValue(row, ['Spend', 'Cost']));
      groups[type].sales += parseNumericValue(getValue(row, ['Sales', 'Revenue']));
      groups[type].clicks += parseNumericValue(getValue(row, ['Clicks']));
      groups[type].orders += parseNumericValue(getValue(row, ['Orders']));
      groups[type].impressions += parseNumericValue(getValue(row, ['Impressions']));
    });
              ...
              <div className="border-2 border-gray-200 rounded-lg p-6 bg-gradient-to-br from-gray-50 to-white">
                <div className="grid grid-cols-4 gap-4 mb-6">
                  <div className="text-center p-4 bg-white rounded-lg shadow">
                    ...
                    <div className="text-2xl font-bold text-red-600">$12,453</div>
                  </div>
                  <div className="text-center p-4 bg-white rounded-lg shadow">
                    <div className="text-sm text-gray-600">Total Sales</div>
                    <div className="text-2xl font-bold text-green-600">$54,210</div>
                  </div>
                  <div className="text-center p-4 bg-white rounded-lg shadow">
                    <div className="text-sm text-gray-600">ACoS</div>
                    <div className="text-2xl font-bold text-blue-600">23.0%</div>
                  </div>
                  <div className="text-center p-4 bg-white rounded-lg shadow">
                    <div className="text-sm text-gray-600">ROAS</div>
                    <div className="text-2xl font-bold text-purple-600">4.35x</div>
                  </div>
                </div>
                <div className="text-sm text-gray-500 text-center">Real-time metrics updated as you upload new data</div>
              </div>
            </div>
...
// Expose App to global scope for browser use
window.App = App;
