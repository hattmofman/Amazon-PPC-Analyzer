  ...
  const analyzeData = (rawData) => {
    try {
      ...
      if (validData.length === 0) {
        console.error('No valid data found. First row:', rawData[0]);
      ...
      const totalSpend = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Spend', 'Cost', 'Ad Spend', 'Total Spend'])), 0);
      const totalSales = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Sales', 'Revenue', '7 Day Total Sales', 'Total Sales', 'Sales 7d'])), 0);
      const totalClicks = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Clicks', 'Click'])), 0);
      const totalImpressions = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Impressions', 'Impr.', 'Impr'])), 0);
      const totalOrders = validData.reduce((sum, row) => sum + parseNumericValue(getValue(row, ['Orders', '7 Day Total Orders', 'Total Orders', 'Orders 7d'])), 0);
