...
function App({ session, supabase }) {
  ...
  const deduplicateData = (data) => {
    ...
    // Strategy: Group by campaign + keyword/targeting, then keep only rows with unique dates
    // or if no date info, keep rows that appear to be daily (smallest time periods)
    
    const groupKey = (row) => {
      const campaign = String(getValue(row, ['Campaign Name (Informational only)', 'Campaign Name', 'Campaign'])).trim();
      const keyword = String(getValue(row, ['Customer Search Term', 'Keyword Text', 'Targeting'])).trim();
      return `${campaign}|${keyword}`;
    };
    
    // Group rows by campaign+keyword
    ...
    console.log('Unique campaign+keyword groups:', groups.size);
    
    // For each group, deduplicate
    const deduplicatedRows = [];
    let duplicatesRemoved = 0;
    
    groups.forEach((rows, key) => {
      if (rows.length === 1) {
        // Only one row for this group, keep it
        deduplicatedRows.push(rows[0]);
        return;
      }
      
      // Multiple rows - need to deduplicate
      // Check if rows have different date ranges to identify daily vs weekly vs monthly
      const rowsWithDates = rows.map(row => {
        const startDate = getValue(row, ['Start Date', 'Start date', 'Date', 'Start']);
        const endDate = getValue(row, ['End Date', 'End date', 'Date', 'End']);
        return { row, startDate, endDate };
      });
      
      // If we have date information, keep only unique date ranges
      const hasDateInfo = rowsWithDates.some(r => r.startDate || r.endDate);
      
      if (hasDateInfo) {
        // Keep rows with unique date ranges
        const uniqueDateRanges = new Map();
        rowsWithDates.forEach(({ row, startDate, endDate }) => {
          const dateKey = `${startDate}|${endDate}`;
          if (!uniqueDateRanges.has(dateKey)) {
            uniqueDateRanges.set(dateKey, row);
          }
        });
        deduplicatedRows.push(...uniqueDateRanges.values());
        duplicatesRemoved += rows.length - uniqueDateRanges.size;
      } else {
        // No date info - look for exact duplicate metrics
        ...
        rows.forEach(row => {
          ...
          const metricKey = `${spend}|${sales}|${clicks}|${impressions}|${orders}`;
          
          if (!uniqueMetrics.has(metricKey)) {
            uniqueMetrics.set(metricKey, row);
          }
        });
        
        deduplicatedRows.push(...uniqueMetrics.values());
        duplicatesRemoved += rows.length - uniqueMetrics.size;
      }
    });
