...
function App({ session, supabase }) {
  ...
  const deduplicateData = (data) => {
    ...
    // Amazon advertising data has hierarchical levels:
    // - Campaign (highest rollup)
    // - Ad Group (mid-level rollup)
    // - Product Ad / Product Targeting / Keywords (most granular)
    // 
    // Strategy: Keep ONLY the most granular level to avoid counting the same spend multiple times
    
    // Step 1: Identify the entity/operation type
    const entityType = (row) => {
      const entity = String(getValue(row, ['Entity', 'Record Type', 'Operation'])).trim().toLowerCase();
      return entity;
    };
    
    // Step 2: Categorize rows by granularity
    const granularEntities = new Set([
      'product ad',
      'product targeting',
      'keyword',
      'search term',
    ...
    console.log('Entity type distribution:', entityCounts);
    
    // Step 3: Determine if we have mixed hierarchical data
    const hasGranularData = data.some(row => {
      const entity = entityType(row);
      return granularEntities.has(entity);
    });
    
    const hasRollupData = data.some(row => {
      const entity = entityType(row);
      return rollupEntities.has(entity);
    });
    
    console.log('Has granular data:', hasGranularData);
    console.log('Has rollup data:', hasRollupData);
    
    let filteredData;
    
    if (hasGranularData && hasRollupData) {
      // Mixed data - keep only granular rows to avoid double counting
      console.log('Mixed hierarchical data detected - keeping only granular rows');
      filteredData = data.filter(row => {
        const entity = entityType(row);
        return granularEntities.has(entity);
      });
      console.log('Removed', data.length - filteredData.length, 'rollup rows');
    } else if (hasRollupData && !hasGranularData) {
      // Only rollup data - still need to deduplicate by campaign ID
      console.log('Only rollup data found - will deduplicate by campaign ID');
      filteredData = data;
    } else {
      // Only granular data or unclear - keep all
      console.log('Granular data only or unable to determine hierarchy');
      filteredData = data;
    }
    
    // Step 4: Remove exact duplicates from the filtered data
    const uniqueRows = [];
    const seenRows = new Set();
    
    filteredData.forEach(row => {
      const campaignId = String(getValue(row, ['Campaign ID', 'Campaign Id', 'CampaignId'])).trim();
      const adGroupId = String(getValue(row, ['Ad Group ID', 'AdGroup ID', 'AdGroupId'])).trim();
      const keyword = String(getValue(row, ['Customer Search Term', 'Keyword Text', 'Targeting', 'Keyword'])).trim();
      const productTargetId = String(getValue(row, ['Product Targeting ID'])).trim();
      const adId = String(getValue(row, ['Ad ID'])).trim();
      const startDate = String(getValue(row, ['Start Date', 'Start date'])).trim();
      const endDate = String(getValue(row, ['End Date', 'End date'])).trim();
      const spend = parseNumericValue(getValue(row, ['Spend', 'Cost']));
      const sales = parseNumericValue(getValue(row, ['Sales', 'Revenue']));
      
      const signature = `${campaignId}|${adGroupId}|${keyword}|${productTargetId}|${adId}|${startDate}|${endDate}|${spend}|${sales}`;
    ...
    console.log('After removing exact duplicates:', uniqueRows.length);
    
    // Step 5: If we still have date range duplicates (daily/weekly/monthly), keep the most granular
    ...
    uniqueRows.forEach(row => {
      ...
      const key = `${campaignId}|${adGroupId}|${keyword}|${productTargetId}|${adId}`;
    ...
    let dateRangeDuplicatesRemoved = 0;
    ...
    groups.forEach((rows, key) => {
      ...
      // Multiple rows for same item - check date ranges
      const rowsWithSpan = rows.map(row => {
        const startDateStr = getValue(row, ['Start Date', 'Start date']);
        const endDateStr = getValue(row, ['End Date', 'End date']);
        ...
        if (startDateStr && endDateStr) {
          try {
            ...
            span = Math.abs(endDate - startDate) / (1000 * 60 * 60 * 24);
          } catch (e) {
            span = 999;
          }
        } else {
      ...
      const minSpan = Math.min(...rowsWithSpan.map(r => r.span));
      const granularRows = rowsWithSpan.filter(r => r.span === minSpan);
      
      deduplicatedRows.push(...granularRows.map(r => r.row));
      dateRangeDuplicatesRemoved += rows.length - granularRows.length;
    });
    
    console.log('Output rows:', deduplicatedRows.length);
    console.log('Date range duplicates removed:', dateRangeDuplicatesRemoved);
    console.log('Total rows removed:', data.length - deduplicatedRows.length);
